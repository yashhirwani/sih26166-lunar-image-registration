...
def create_spatial_heatmap(points, img_shape, grid_size=8, background_img=None, alpha=0.55):
    """
    Renders inlier spatial density as a heatmap, addressing ISRO's uniform-
    distribution requirement visually (a spatial_score number alone doesn't
    show a judge *where* coverage is thin).

    Parameters:
        points        : (N, 2) inlier (x, y) coordinates
        img_shape     : (height, width) of the image the points are in
        grid_size     : grid rows/cols — pass the same value used for
                        compute_coverage_metrics() so the heatmap matches
                        the reported cell counts
        background_img: optional grayscale/BGR image to blend the heatmap
                        over (e.g. the reference image); if None, the
                        heatmap is rendered on a plain dark background
        alpha         : heatmap opacity when blended over background_img

    Returns:
        BGR image, same (height, width) as img_shape
    """
    h, w = img_shape[:2]
    cell_h = h / grid_size
    cell_w = w / grid_size
    ...
    # Upsample the coarse grid to full image resolution with smooth
    # interpolation so it reads as a heatmap, not a blocky grid.
    density_full = cv2.resize(density_norm, (w, h), interpolation=cv2.INTER_CUBIC)
    density_full = np.clip(density_full, 0, 255).astype(np.uint8)
    heatmap = cv2.applyColorMap(density_full, cv2.COLORMAP_JET)

    if background_img is not None:
        bg = background_img
        if len(bg.shape) == 2:
            bg = cv2.cvtColor(bg, cv2.COLOR_GRAY2BGR)
        bg = cv2.resize(bg, (w, h)) if bg.shape[:2] != (h, w) else bg
        blended = cv2.addWeighted(heatmap, alpha, bg, 1 - alpha, 0)
    else:
        blended = heatmap

    # Draw grid lines and per-cell counts for an inspectable, not just
    # pretty, heatmap.
    for i in range(1, grid_size):
        x = int(i * cell_w)
        y = int(i * cell_h)
        cv2.line(blended, (x, 0), (x, h), (80, 80, 80), 1)
        cv2.line(blended, (0, y), (w, y), (80, 80, 80), 1)

    font = cv2.FONT_HERSHEY_SIMPLEX
    for row in range(grid_size):
        for col in range(grid_size):
            count = int(density[row, col])
            if count == 0:
                continue
            cx = int((col + 0.5) * cell_w)
            cy = int((row + 0.5) * cell_h)
            cv2.putText(blended, str(count), (cx - 6, cy + 4), font, 0.4,
